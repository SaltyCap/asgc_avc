...
class MotorInterface:
    ...
    def start(self, nav_controller=None):
        ...
        if not os.access(motor_path, os.X_OK):
            print(f"ERROR: Motor control program is not executable: {motor_path}")
            return False
    ...
    def stop(self):
        ...
        try:
            ...
            print(f"Warning: failed to send graceful shutdown commands: {e}")
    ...
    def send_command(self, command):
        ...
        if not self._is_process_alive():
            ...
            if now - self._last_offline_warning_ts > 2.0:
                print(f"[MOTOR] Dropping command while motor process is offline: {command}")
        ...
        try:
            ...
            print("[MOTOR] Command queue full; dropping command")
            return False
    ...
    def _read_errors(self):
        ...
        while self.running:
            try:
                ...
                if line:
                    print(f"[MOTOR STDERR] {line.strip()}")
                elif process.poll() is not None:
                    self.running = False
                    break
            except Exception as e:
                print(f"Error reading motor stderr: {e}")
                self.running = False
                break
    ...
    def _resolve_motor_path(self):
        ...
        return candidates[0], project_root
