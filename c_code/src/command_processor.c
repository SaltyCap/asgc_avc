...
static int parse_pid_profile(const char* token, PidProfile* out_profile) {
    if (!token || !out_profile) {
        return 0;
    }

    if (strcasecmp(token, "drive") == 0 || strcasecmp(token, "straight") == 0) {
        *out_profile = PID_PROFILE_DRIVE;
        return 1;
    }
    if (strcasecmp(token, "turn") == 0 || strcasecmp(token, "turning") == 0) {
        *out_profile = PID_PROFILE_TURN;
        return 1;
    }
    return 0;
}

static const char* pid_profile_name(PidProfile profile) {
    return (profile == PID_PROFILE_TURN) ? "turn" : "drive";
}

static int parse_nav_controller_mode(const char* token, NavControllerMode* out_mode) {
    if (!token || !out_mode) {
        return 0;
    }
    if (strcasecmp(token, "pid") == 0) {
        *out_mode = NAV_CONTROLLER_PID;
        return 1;
    }
    if (strcasecmp(token, "ml") == 0 || strcasecmp(token, "model") == 0) {
        *out_mode = NAV_CONTROLLER_ML;
        return 1;
    }
    return 0;
}

static const char* nav_controller_mode_name(NavControllerMode mode) {
    return (mode == NAV_CONTROLLER_ML) ? "ml" : "pid";
}

static void print_pid_profile(PidProfile profile) {
    double kp = 0.0, ki = 0.0, kd = 0.0, ka = 0.0, vstop = 0.0;
    if (!control_get_pid_gains(profile, &kp, &ki, &kd, &ka, &vstop)) {
        return;
    }
    printf("PID %s %.6f %.6f %.6f %.6f %.6f\n",
           pid_profile_name(profile), kp, ki, kd, ka, vstop);
    fflush(stdout);
}

static void start_relative_turn(double degrees) {
    ...
    nav_ctrl.target_x = odometry.x;
    nav_ctrl.target_y = odometry.y;
    nav_ctrl.target_heading = odometry.heading + degrees;
    nav_ctrl.isolated_turn = 1;
    while (nav_ctrl.target_heading >= 360.0) nav_ctrl.target_heading -= 360.0;
    while (nav_ctrl.target_heading < 0.0) nav_ctrl.target_heading += 360.0;
    ...
    int32_t counts = calculate_turn_counts(degrees);
...
void process_command(char* cmd) {
    ...
    if (strncasecmp(cmd, "goto", 4) == 0) {
        ...
        double degrees = 0.0;
        if (sscanf(cmd + 4, "%lf", &degrees) == 1) {
            if (degrees > 360.0) degrees = 360.0;
            if (degrees < -360.0) degrees = -360.0;
            if (fabs(degrees) < 0.1) {
                printf("ERR turn expects |degrees| >= 0.1\n");
                fflush(stdout);
                return;
            }
            start_relative_turn(degrees);
        } else {
            printf("ERR turn <degrees>\n");
            fflush(stdout);
        }
    } else if (strncasecmp(cmd, "pid_set", 7) == 0) {
        char profile_token[16];
        double kp = 0.0, ki = 0.0, kd = 0.0, ka = 0.0, vstop = 0.0;
        PidProfile profile = PID_PROFILE_DRIVE;
        if (sscanf(cmd + 7, "%15s %lf %lf %lf %lf %lf",
                   profile_token, &kp, &ki, &kd, &ka, &vstop) == 6 &&
            parse_pid_profile(profile_token, &profile)) {
            if (!control_set_pid_gains(profile, kp, ki, kd, ka, vstop)) {
                printf("ERR pid_set invalid values\n");
                fflush(stdout);
                return;
            }
            printf("OK pid_set %s %.6f %.6f %.6f %.6f %.6f\n",
                   pid_profile_name(profile), kp, ki, kd, ka, vstop);
            fflush(stdout);
            print_pid_profile(profile);
        } else {
            printf("ERR pid_set <drive|turn> <kp> <ki> <kd_vel> <ka_accel> <v_stop>\n");
            fflush(stdout);
        }
    } else if (strncasecmp(cmd, "pid_get", 7) == 0) {
        char profile_token[16];
        PidProfile profile = PID_PROFILE_DRIVE;
        if (sscanf(cmd + 7, "%15s", profile_token) == 1) {
            if (!parse_pid_profile(profile_token, &profile)) {
                printf("ERR pid_get <drive|turn>\n");
                fflush(stdout);
                return;
            }
            print_pid_profile(profile);
        } else {
            print_pid_profile(PID_PROFILE_TURN);
            print_pid_profile(PID_PROFILE_DRIVE);
        }
    } else if (strncasecmp(cmd, "nav_controller", 14) == 0) {
        char mode_token[16];
        NavControllerMode mode = NAV_CONTROLLER_PID;
        if (sscanf(cmd + 14, "%15s", mode_token) == 1) {
            if (!parse_nav_controller_mode(mode_token, &mode)) {
                printf("ERR nav_controller <pid|ml>\n");
                fflush(stdout);
                return;
            }
            if (!control_set_nav_controller_mode(mode)) {
                printf("ERR nav_controller failed\n");
                fflush(stdout);
                return;
            }
            printf("OK nav_controller %s\n", nav_controller_mode_name(mode));
            fflush(stdout);
        }
        mode = control_get_nav_controller_mode();
        printf("NAV_CONTROLLER %s\n", nav_controller_mode_name(mode));
        fflush(stdout);
        ...
        /* Stop the 20kHz logging thread BEFORE dump_log()/init_log_system().
         * Without this, the logging thread can write into the freed log_buffer
         * between dump_log()'s free() and init_log_system()'s malloc(), which
         * corrupts the malloc heap and causes an abort. */
        stop_logging_thread();
        dump_log();
        init_log_system();
        ...
        control_set_nav_controller_mode(NAV_CONTROLLER_ML);
        ...
        printf("STATUS %.2f %.2f %.2f %d\n", status_x, status_y, status_h, status_state);
        fflush(stdout);
    } else if (strncasecmp(cmd, "uart_servo", 10) == 0) {
        unsigned int low = 0, high = 0;
        if (sscanf(cmd + 10, " 0x%02X 0x%02X", &low, &high) == 2 ||
            sscanf(cmd + 10, " %u %u", &low, &high) == 2) {
            if (uart_send_servo_bytes((uint8_t)(low & 0x7Fu), (uint8_t)(high & 0x7Fu)) == 0) {
                printf("OK uart_servo 0x%02X 0x%02X\n", low & 0x7Fu, high & 0x7Fu);
            } else {
                printf("ERR uart_servo write failed\n");
            }
            fflush(stdout);
        } else {
            printf("ERR uart_servo <low_hex> <high_hex>\n");
            fflush(stdout);
        }
    }
}
