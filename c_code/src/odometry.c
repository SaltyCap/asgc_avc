...
#define MIN_ENCODER_DT_SEC 0.0005
#define MAX_ENCODER_DT_SEC 0.1
#define VELOCITY_LPF_ALPHA 0.35
#define ACCEL_LPF_ALPHA 0.30

void update_encoder_rotation(EncoderState *enc, int16_t raw_angle, int motor_id, double sample_time) {
    if (enc->last_raw_angle < 0) {
        enc->last_raw_angle = raw_angle;
        enc->current_raw_angle = raw_angle;
        enc->total_counts = 0;
        enc->velocity_counts_per_sec = 0.0;
        enc->acceleration_counts_per_sec2 = 0.0;
        enc->last_velocity_counts_per_sec = 0.0;
        enc->last_update_time = sample_time;
        return;
    }
    ...
    int32_t signed_delta = encoder_sign * delta;
    enc->total_counts += signed_delta;
    enc->last_raw_angle = raw_angle;
    enc->current_raw_angle = raw_angle;

    double dt = sample_time - enc->last_update_time;
    if (dt < MIN_ENCODER_DT_SEC) dt = MIN_ENCODER_DT_SEC;
    if (dt > MAX_ENCODER_DT_SEC) dt = MAX_ENCODER_DT_SEC;

    double instant_velocity = signed_delta / dt;
    double filtered_velocity = (VELOCITY_LPF_ALPHA * instant_velocity) +
                               ((1.0 - VELOCITY_LPF_ALPHA) * enc->last_velocity_counts_per_sec);

    double instant_accel = (filtered_velocity - enc->last_velocity_counts_per_sec) / dt;
    double filtered_accel = (ACCEL_LPF_ALPHA * instant_accel) +
                            ((1.0 - ACCEL_LPF_ALPHA) * enc->acceleration_counts_per_sec2);

    enc->velocity_counts_per_sec = filtered_velocity;
    enc->acceleration_counts_per_sec2 = filtered_accel;
    enc->last_velocity_counts_per_sec = filtered_velocity;
    enc->last_update_time = sample_time;
}
...
void* encoder_feedback_thread(void* arg) {
    ...
    while (running) {
        ...
        double sample_time = sensors.timestamp;
        ...
        if (left_angle >= 0) {
            ...
            update_encoder_rotation(&encoders[0], left_angle, 0, sample_time);
            pthread_mutex_unlock(&motors[0].lock);
        }

        if (right_angle >= 0) {
            pthread_mutex_lock(&motors[1].lock);
            update_encoder_rotation(&encoders[1], right_angle, 1, sample_time);
...
void update_odometry(void) {
    ...
    // Use the dead zone measured at startup calibration instead of a hardcoded constant.
    if (fabs(gyro_rate) < g_gyro_rate_dead_zone) {
        gyro_rate = 0.0;
    }

    // Integrate gyro heading only while encoders report movement.
    double delta_heading = 0.0;
    if (d_left != 0 || d_right != 0) {
        delta_heading = gyro_rate * dt;
    }
