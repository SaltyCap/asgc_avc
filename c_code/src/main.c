...
int main(void) {
    ...
    // Explicitly set neutral pulse for ESC calibration
    for (int i = 0; i < 2; i++) {
    ...
    // Initialize UART for scoop servo control (optional, non-fatal).
    if (uart_init() < 0) {
        fprintf(stderr, "WARNING: UART servo init failed (%s). Scoop control unavailable.\n", UART_DEVICE);
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        encoders[i].velocity_counts_per_sec = 0.0;
        encoders[i].acceleration_counts_per_sec2 = 0.0;
        encoders[i].last_velocity_counts_per_sec = 0.0;
        encoders[i].last_update_time = 0.0;
        encoders[i].target_counts = 0;
        encoders[i].has_target = 0;
        encoders[i].ramp_start_time = 0.0;
    }

    fprintf(stderr, "INFO: Arming ESCs (5s)...\n");
    fflush(stderr);
    sleep(5);
    ...
    if (shutdown_signal_received) {
        fprintf(stderr, "INFO: Shutdown requested by signal, flushing logs.\n");
    }
    stop_logging_thread();
    dump_log();
